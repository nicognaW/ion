...
func (p *Project) Run(ctx context.Context, input *StackInput) error {
	...
	defer func() {
		...
		multi := io.MultiWriter(oldTypesFile)
		...
		packageJsons := fs.FindDown(p.PathRoot(), "package.json")
		...
		for _, packageJson := range packageJsons {
			...
			if data.DevDependencies["sst"] == "" && data.Dependencies["sst"] == "" {
				continue
			}
			...
			if data.Dependencies["@cloudflare/workers-types"] != "" || data.DevDependencies["@cloudflare/workers-types"] != "" {
				...
				envFile.WriteString("  export interface Resource " + inferTypes(nonCloudflareLinks, "  ") + "\n")
				envFile.WriteString("}" + "\n")
				...
				if len(bindings) > 0 {
					...
					envFile.WriteString("declare module \"sst\" {\n")
					envFile.WriteString("  export interface Resource " + inferTypes(bindings, "  ") + "\n")
					envFile.WriteString("}\n")
				}
				continue
			}
			envFile.Write([]byte("declare module \"sst\" {\n"))
			envFile.Write([]byte("  export interface Resource " + inferTypes(complete.Links, "  ") + "\n"))
			envFile.Write([]byte("}\n"))
		}
